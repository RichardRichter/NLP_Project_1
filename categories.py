...
class CategoryExtractor():
    ...
    def __init__(self, tweets):
        ...
        self.patterns = {
            ...
            "specific": [r'.*\bnominated for (best\b.*)', r'.*\bw[io]ns? (best\b.*)', r'.*\bawarded (best\b.*)',
                         r'.*\baward for (best\b.*)', r'.*\bawarded the (best\b.*)', r'.*\bwinner of (best\b.*)',
                         r'.*\b(best\b.*)goes to\b.*', r'.*\b(best\b.*)awarded to\b.*', r'.*\b(best\b.*)won by\b.*'],
    ...
    def find_components(self, tweets, limit=4, remove_best=False):
        ...
        for n in range(2, limit + 1):
            ...
            components += [self.Component(engram) for engram in engrams[n].most_common(50) if
                           engram[0][0] != 'or' and engram[0][-1] != 'or']
    ...
    def score_components(self, components, tweets, remove_best=False):
        ...
        components = components if components else (
            self.components if self.components else self.find_components(tweets))
